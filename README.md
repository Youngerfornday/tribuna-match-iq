...
- Each artwork is tuned to the block it sits in: the hero card and the prize banner have different backgrounds, so each image gets its own levels and its own radial mask, and both bleed off their block's edge. Forcing one shared treatment on both made the banner read as a pasted rectangle.
